...
dat.taxa <-read.csv("output/fssgam - habitat-lidar/Parks-Geographe-synthesis_all.var.imp.csv") %>% #from local copy
  rename(resp.var=X)%>%
  ...
  mutate(label=ifelse(predictor=="roughness"&resp.var=="Sessile invertebrates","X",label))%>%
  mutate(label=ifelse(predictor=="Z"&resp.var=="Sessile invertebrates","X",label))%>%
  dplyr::mutate(importance = ifelse(resp.var == "Macroalgae" & predictor %in% c("Z"), 
                                    importance * -1, importance)) %>%
  dplyr::mutate(importance = ifelse(resp.var == "Sand" & predictor %in% c("detrended", "Z", "roughness"), 
                                    importance * -1, importance)) %>%
  dplyr::mutate(importance = ifelse(resp.var == "Seagrass" & predictor %in% c("roughness"), 
                                    importance * -1, importance)) %>%
...
gg.importance.scores <- ggplot(dat.taxa, aes(x=predictor,y=resp.var,fill=importance)) +
   ...
   scale_fill_gradientn(legend_title, colours=c(re), na.value = "grey98",
                         limits = c(-1, 1.01))+                                 # Changed to 1.01 - importance score greater than 1?
