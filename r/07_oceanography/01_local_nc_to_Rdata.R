...
nc_file_to_get_sla <- open.nc("data/spatial/oceanography/large/IMOS_aggregation_20230118T011935Z_SLA_geographe.nc",write = TRUE)
...
rm(list= ls()[!(ls() %in% c('locations', 'Zone','locs','Lon_w',
                            'Lon_e','Lat_n','Lat_s'))])
gc()
######### SST #########
#IMOS - SRS - SST - L3S - Single Sensor - 6 day - day and night time - Australia
nc_file_to_get_sst <- open.nc("data/spatial/oceanography/large/IMOS_aggregation_20230118T011543Z_SST_geographe.nc",write = TRUE)
...
# There isn't any acidification data available for this area
...
nc_file_to_get_dhw <- open.nc("data/spatial/oceanography/large/dhw_5km_2e11_2ce8_27b9_U1674005047127_geographe.nc",write = TRUE)
