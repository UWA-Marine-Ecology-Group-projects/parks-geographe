...
dat2 <- read.csv(paste("output/fssgam - fish-lidar", paste(name, "length_all.var.imp.csv", sep = "_"), sep = "/")) %>% 
  dplyr::rename(resp.var=X)%>%
  gather(key=predictor,value=importance,2:ncol(.))%>%
  dplyr::mutate(importance = ifelse(predictor %in% c("Z", "seagrass", "mean.relief") & resp.var == "greater than legal size", 
                                    importance * -1, importance)) %>%
  dplyr::mutate(importance = ifelse(predictor %in% c("detrended", "Z", "mean.relief") & resp.var == "smaller than legal size", 
                                    importance * -1, importance)) %>%
  glimpse()
...
dat.taxa <- dat %>%
  ...
  mutate(label=ifelse(predictor=="Z"&resp.var=="greater than legal size","X",label))%>%
  mutate(label=ifelse(predictor=="detrended"&resp.var=="smaller than legal size","X",label))%>%
  mutate(label=ifelse(predictor=="inverts"&resp.var=="smaller than legal size","X",label))%>%
  glimpse()
